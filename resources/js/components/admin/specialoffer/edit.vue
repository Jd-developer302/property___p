<template>
        ...
        <div class="col-lg-12">
          ...
          <div class="card shadow mb-4">
            ...
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Edit Special Offer</h6>
            </div>
  ...
  const specialOfferId = route.params.id; // Get MasterPlan ID from route params
  ...
  const fetchMasterPlan = async () => {
    try {
      const { data } = await axios.get(`/api/admin/specialoffers/${specialOfferId}`);
      form.value = {
        ...data,
  ...
  const submitForm = async () => {
    ...
    try {
      await axios.post(`/api/admin/specialoffers-update/${specialOfferId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      router.push('/admin/specialoffers'); // Redirect after successful update
    } catch (error) {
