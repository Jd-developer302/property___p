<template>
  ...
  const route = useRoute(); // Get the feature ID from the route
  ...
  const fetchFeature = async () => {
    try {
      const featureId = route.params.id; // Get the feature ID from route
      const response = await axios.get(`/api/admin/features/${featureId}`);
      form.value = response.data;
      nextTick(() => {
        $('#description').summernote('code', form.value.description); // Populate description field
      });
    } catch (error) {
  ...
  const submitForm = async () => {
    ...
    try {
      const featureId = route.params.id; // Get the feature ID from the route
      await axios.post(`/api/admin/features-update/${featureId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      router.push('/admin/features'); // Redirect after successful update
    } catch (error) {
  ...
  onMounted(() => {
    fetchFeature(); // Fetch the existing feature for editing
