<template>
    <Base>
      ...
      <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Create Project Location</h1>
        <li class="breadcrumb-item" style="list-style: none;">
          <router-link class="btn btn-primary" to="/admin/projectlocation">- Back</router-link>
        </li>
      </div>
      ...
      <div class="row">
        <div class="col-lg-12">
          <!-- Card for Creating FloorPlan -->
          <div class="card shadow mb-4">
            <!-- Card Header -->
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Create Project Location</h6>
            </div>
  ...
  // Submit form to create a new floorplan
  const submitForm = async () => {
    ...
    try {
      await axios.post('/api/admin/projectlocations', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      router.push('/admin/projectlocation'); // Redirect after successful creation
    } catch (error) {
  ...
  onMounted(() => {
    ...
    nextTick(() => {
      $('#description').summernote({
        placeholder: 'Enter floorplan description here...',
        tabsize: 2,
        height: 200,
      });
    });
