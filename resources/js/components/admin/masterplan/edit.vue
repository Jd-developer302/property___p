<template>
    <Base>
      ...
      <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit MasterPlan</h1>
        ...
        <div class="col-lg-12">
          ...
          <div class="card shadow mb-4">
            ...
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Edit MasterPlan</h6>
            </div>
  ...
  const masterPlanId = route.params.id; // Get MasterPlan ID from route params
  ...
  const fetchMasterPlan = async () => {
    try {
      const { data } = await axios.get(`/api/admin/masterplans/${masterPlanId}`);
      form.value = {
        ...data,
  ...
  const submitForm = async () => {
    ...
    try {
      await axios.post(`/api/admin/masterplans-update/${masterPlanId}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      router.push('/admin/masterplans'); // Redirect after successful update
    } catch (error) {
