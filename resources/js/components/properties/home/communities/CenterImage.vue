<template>
   ...
   <a :href="`/communities/${selectedCommunity.slug}`">   <img :src="getImageUrl(selectedCommunity.image)" alt="Community Image" style="height: 150%" /></a>
    ...
    methods: {
      async fetchCommunities(url = '/api/all_communities') {
        try {
          ...
          // Set a default community to display
          if (this.communities.data.length > 0) {
            this.selectedCommunity = this.communities.data[0]; // Display the first community's image
          }
        } catch (error) {
