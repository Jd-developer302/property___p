            ...
            <div class="row">
                <div class="col text-center">
                    <div class="my-3 title">
                        <h2>New Launches by {{ developer.name }}</h2>
                    </div>
                </div>
            </div>
            <div class="row">
                <div v-for="project in projects" :key="project.id" class="col-lg-3 col-md-3 col-sm-12 mb-4">
                    <div class="list-grid-layout modern-layout small">
                        <div class="grid-layout-header">
                            <a :href="`/projects/${project.slug}`" class="list-thumb">
                                <img :src="`/storage/${project.image}`" class="img-fluid mx-auto" :alt="project.name" />
                            </a>
                        </div>
                        <div class="grid-layout-body">
                            <div class="gl-list-caption">
                                <h4 class="mt-0">
                                    <a :href="`/projects/${project.slug}`" class="list-title">{{ project.name }}</a>
                                </h4>
                                <a :href="`/properties-for-sale-at-${project.community ? project.community.slug : 'unknown'}`"
                                    class="gl-tagline">
                                    {{ project.community ? project.community.name : 'Unknown Community' }}
                                </a>
                            </div>
                            ...
                            <div class="ft-loke">
                                <a :href="`/projects/${project.slug}`" class="btn-hover default-btn">View Details</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        async created() {
            try {
                const slug = this.$route.params.slug; // Assuming you're using Vue Router and the slug is in the route params
                const response = await axios.get(`/api/developers/${slug}/new-launches`);
...
<style scoped>
    ...
    *,
    ...
    .list-grid-layout:focus a.list-thumb,
    .list-grid-layout:hover a.list-thumb {
