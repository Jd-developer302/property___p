<script setup>
    ...
    onMounted(async () => {
      ...
      try {
        const response = await axios.get(`/api/masterplan/${slug}`);
        console.log("API response:", response.data);
        masterplan.value = response.data;
      } catch (error) {
                ...
                <div v-else>
                  ...
                  <section class="similar-properties card mt-md-5 block">
                    <h2 class="h4 has-line">
                      Recent Projects by {{ masterplan.project?.name || 'Loading...' }}
                    </h2>
      
                    <div class="row">
                      <!-- Check if feature.project is defined -->
                      <div v-if="masterplan.project" class="col-md-4">
                        <div class="property mb-lg-0">
                          <div class="image">
                            <!-- Use feature.project.image safely -->
                            <img
                              :src="masterplan.project.image ? `/storage/${masterplan.project.image}` : '/images/default-image.png'"
                              width="800"
                              height="450"
                              :alt="masterplan.project.name"
                              class="img-fluid cover"
                            />
                            <div class="overlay d-flex align-items-center justify-content-center">
                              <a :href="`/projects/${masterplan.project.id}`" class="btn btn-light btn-sm">View Details</a>
                            </div>
                          </div>
                          <div class="info">
                            <a :href="`/projects/${masterplan.project.id}`" class="no-anchor-style">
                              <h3 class="text-thin mb-1">{{ masterplan.project.name }}</h3>
                            </a>
                          </div>
                        </div>
                      </div>
                      <!-- Display a message if no project is available -->
                      <div v-if="!masterplan.project" class="col-md-12">
...
<style scoped>
    *,
    ...
    .property-single-features ol,
    .property-single-features ul {
        color: #444;
        font-size: 16px;
        font-weight: 400
    }
    ...
    nav.navbar .es .nav-link,
    nav.navbar .ru .nav-link {
        letter-spacing: 0;
        margin-right: 0
    }
    ...
    .floor-plan a,
    .payment-plan a,
    .property-single-description a,
    .property-single-features a,
    .property-single-map a {
        color: #585858
    }

    .floor-plan p a,
    .payment-plan p a,
    .property-single-features p a,
    .property-single-map p a {
        font-weight: 600
    }
    ...
    .property-single-map a.nav-link {
        border: 0;
        border-radius: 50px;
        font-size: .85rem;
        padding: 5px 20px;
    ...
    .property-single-map a.nav-link.active {
        background: #f3c84b;
        color: #fff
    }
